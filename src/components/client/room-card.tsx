...
export const RoomCard = ({ room, currentUser }: RoomCardProps) => {
  ...
  // 2. Logic so sánh (Compare Store)
  const { ids, addRoom, removeRoom } = useCompareStore();
  const [isMounted, setIsMounted] = useState(false);
  useEffect(() => setIsMounted(true), []);
  const isCompared = isMounted ? ids.includes(room.id) : false;

  const handleToggleCompare = (e: React.MouseEvent) => {
    e.preventDefault(); 
    e.stopPropagation();
    isCompared ? removeRoom(room.id) : addRoom(room.id);
  };
      ...
      <div className="relative h-[240px] w-full overflow-hidden">
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2 z-20">
            ...
            <button
                ...
                className={`w-[42px] h-[42px] rounded-full shadow-lg transition-all duration-300 flex items-center justify-center group/btn
                    ${isCompared 
                        ? "bg-blue-600 text-white hover:bg-blue-700 scale-105" 
                        : "bg-white/90 text-slate-500 hover:text-blue-600 hover:bg-white"
                    }
                `}
                title={isCompared ? "Bỏ so sánh" : "Thêm vào so sánh"}
            >
                {isCompared ? (
                    <Check className="w-5 h-5 animate-in zoom-in spin-in-180 duration-300" />
                ) : (
                    <Scale className="w-5 h-5 transition-transform group-hover/btn:rotate-12" />
                )}
            </button>
        </div>
