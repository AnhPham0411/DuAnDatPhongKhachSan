...
export default async function MyBookingsPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      <h1 className="text-2xl font-bold mb-6">Lịch sử đặt phòng của tôi</h1>
          ...
          {bookings.map((booking) => (
            <div key={booking.id} className="flex flex-col md:flex-row border rounded-xl overflow-hidden bg-white shadow-sm hover:shadow-md transition">
               ...
               <div className="w-full md:w-56 h-48 md:h-auto relative bg-gray-100 shrink-0">
                  <Image 
                    src={booking.room.images[0]?.url || "/images/placeholder.jpg"}
               ...
               <div className="flex-1 p-5 flex flex-col justify-between">
                  ...
                  <div className="mt-4 grid grid-cols-2 md:grid-cols-3 gap-4 text-sm border-t pt-4">
                     <div>
                        <p className="text-gray-500 text-xs uppercase font-semibold">Nhận phòng</p>
                        <p className="font-medium">{format(booking.checkIn, "dd 'thg' MM, yyyy", { locale: vi })}</p>
                     </div>
                     <div>
                        <p className="text-gray-500 text-xs uppercase font-semibold">Trả phòng</p>
                        <p className="font-medium">{format(booking.checkOut, "dd 'thg' MM, yyyy", { locale: vi })}</p>
                     </div>
               ...
               <div className="p-4 bg-gray-50 flex flex-row md:flex-col justify-center gap-2 border-t md:border-t-0 md:border-l w-full md:w-40 shrink-0">
                  <Link href={`/rooms/${booking.roomId}`} className="w-full">
